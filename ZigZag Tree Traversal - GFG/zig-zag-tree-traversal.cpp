...
class Solution{
    ...
    void preLtoR(Node* root,int currLevel,int reqLevel,vector<int> &temp){
        if(root==NULL) return ;
    ...
    void preRtoL(Node* root,int currLevel,int reqLevel,vector<int> &temp){
        if(root==NULL) return ;
        if(currLevel==reqLevel) {
            temp.push_back(root->data);
            return ;
    ...
    vector <int> zigZagTraversal(Node* root)
    {
        ...
        for(int i=1; i<=n; i++){
            if(i%2==0) preRtoL(root,1,i,ans);
            else preLtoR(root,1,i,ans);
